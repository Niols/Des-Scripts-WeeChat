...
SCRIPT_VERSION = "0.2"
SCRIPT_LICENSE = "GPL3"
SCRIPT_DESCRIPTION = "Helps you shortening your URLs, wether you want them replaced in outgoing/incoming messages or shortened under the incoming messages."
...
if weechat.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, SCRIPT_LICENSE, SCRIPT_DESCRIPTION, "", ""):
...
# For every message, if it's not your message, and if the `add_under` section is
# activated, prints every shortened URL in the specified color with the
...
def privmsg(data, buf, date, tags, displayed, hilight, prefix, msg):
    ...
    for url in get_urls_to_shorten(msg):
        ...
        if new_url != url:
            weechat.prnt(buf, '%(color)s%(prefix)s%(url)s%(suffix)s%(reset)s' % \
...
# For every incoming message, if the `replace_in` section is activated, replaces
# all URLs by their shortened version.
...
def get_urls_to_shorten(msg):
    ...
    return [url for url in url_regexp.findall(msg) if len(url) > max_url_length]
